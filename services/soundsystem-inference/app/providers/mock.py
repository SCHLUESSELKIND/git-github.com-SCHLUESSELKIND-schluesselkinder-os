...
class MockMusicProvider:
    ...
    async def start(
        ...
        self._results[external_job_id] = self._build_manifest(request)
    ...
    def _build_manifest(self, request: GenerationRequest) -> ArtifactManifest:
        base = f"/tmp/snuffraga/{request.project_id}"
        return ArtifactManifest(
            full_mix_wav=f"{base}/full_mix.wav",
            stems=[
                f"{base}/stems/drums.wav",
                f"{base}/stems/bass.wav",
                f"{base}/stems/vocals.wav",
                f"{base}/stems/fx.wav",
            ],
